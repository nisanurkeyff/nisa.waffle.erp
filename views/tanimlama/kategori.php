                                            ...
                                            <div class="card-body mt-2">
                                                <div class="card-datatable text-nowrap table-responsive">
                                                    <table class="table table-hover table-sm">
                                                        ...
                                                        <tbody>
                                                            <?foreach ($rows_kategoriler as $key => $row) {?>
                                                                <tr>
                                                                    <td><?=($key+1)?></td>
                                                                    <td><?=$row->KATEGORI?></td>
                                                                    <td><?=fncDurumSpan($row->DURUM)?></td>
                                                                    <td>
                                                                        <a href="javascript:;" class="btn btn-primary btn-icon" data-id="<?=$row->ID?>" onclick="fncKategoriBilgisi(this)" data-bs-target="#kategoriDuzenleModal" data-bs-toggle="modal"><i class="ri-pencil-line"></i></a>
                                                                        <a href="javascript:;" class="btn btn-danger btn-icon" data-id="<?=$row->ID?>" onclick="fncKategoriSil(this)"><i class="ri-delete-bin-5-line"></i></a>
                                                                    </td>
                                                                </tr>
                                                            <?}?>
                                                        </tbody>
...
<script type="text/javascript">
    ...
    $("#kategoriEkle").on("submit", function(event) {
        ...
        $.ajax({
            url: "/router.php",
            type: "POST",
            data: $(this).serialize() + "&controller=kategori&action=kategori_ekle",
            dataType: "json",
    ...
    $("#kategoriDuzenle").on("submit", function(event) {
        ...
        $.ajax({
            url: "/router.php",
            type: "POST",
            data: $(this).serialize() + "&controller=kategori&action=kategori_kaydet",
            dataType: "json",
            success: function(response) {
                $.unblockUI();
                if (response.HATA) {
